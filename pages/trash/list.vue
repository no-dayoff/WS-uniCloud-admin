...
<template>
  <view>
    ...
    <view class="uni-container">
      <unicloud-db ref="udb" collection="trash" field="name,category" :where="where" page-data="replace"
        ...
        <uni-table :loading="loading" :emptyText="error.message || '没有更多数据'" border stripe type="selection" @selection-change="selectionChange">
          <uni-tr>
              <uni-th align="center">name</uni-th>  <uni-th align="center">category</uni-th>  
            <uni-th width="204" align="center">操作</uni-th>
          </uni-tr>
          <uni-tr v-for="(item,index) in data" :key="index">
               <uni-td align="center"> {{item.name}} </uni-td>    <uni-td align="center"> {{item.category}} </uni-td>   
            <uni-td align="center">
              <view class="uni-group">
...
<script>
  import { enumConverter } from '../../js_sdk/validator/trash.js';

  const db = uniCloud.database()
  // 表查询配置
  const dbOrderBy = 'category desc' // 排序字段
